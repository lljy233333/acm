...
ll dfs(ll pos,ll state,ll limit)//pos为位数，state表示前一位是否为6，limit表示是否有上界 
{
	...
	if(!limit&&dp[pos][state]!=-1) return dp[pos][state];//如果没有限制且没被访问 就返回这个 ！！！ 
	ll up=limit?a[pos]:9;//有上界则为pos 无上界限制则为9 
	ll ans=0;//可行数结果 
	for(int i=0;i<=up;i++)//到上界结束 
	{
		if(state&&i==2) continue;//前一位是6 并且这一位是2 
	...
	//如果没有限制 也就是前一位没有遍历到最大限制数 那就赋值 访问 转移状态 
	return ans;
}
ll solve(ll x)
{
...
int main()
{
	...
	{
		if(n==0&&m==0) break;
		cout<<solve(m)-solve(n-1)<<endl;//因为每次会从0开始，所以要减去最低限制之前的数 
	}
	return 0;
}
